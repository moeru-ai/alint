...
export const lsp = defineCommand({
  ...
  help: [
    ...
    'The server is cache-first. It publishes diagnostics that are already cached, and it never calls a model on its own. Runs that spend tokens happen only through the `alint.runFile` and `alint.runWorkspace` commands.',
  ].join('\n\n'),
  name: 'lsp',
  options: [
    // NOTICE: LSP clients send --stdio by default. stdio is the only transport alint serves, so
    // accept the flag and ignore it. A rejected flag fails the first connection.
