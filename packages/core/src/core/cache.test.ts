...
describe('cache helpers', () => {
  ...
  it('resets cache files when entries or files are arrays', async () => {
    for (const field of ['entries', 'files'] as const) {
      const root = join(tmpdir(), `alint-cache-array-${field}-${Date.now()}`)
      const cachePath = join(root, '.alintcache')
